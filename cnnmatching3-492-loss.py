...
#########################################################
model_to_truevalue = 0
# #######################################################
# df = pd.read_csv('converted_result_1.csv')
...
df = pd.read_csv('/home/lab125/kk/OPT-SAR/OPT-SAR-Registration-script/Dataset_Label_Test2/Label1/' + imgs_file_name + '_points.csv'  # 读取 CSV 文件
                 , skiprows=3)
locations_1_to_use = df[['LeftX', 'LeftY']].values  # shape (N, 2)
locations_2_to_use = df[['RightX', 'RightY']].values  # shape (N, 2
locations_1_to_use = np.array(locations_1_to_use)
locations_2_to_use = np.array(locations_2_to_use)
...
ind = all_indices if model_to_truevalue else inlier_idxs

# 计算 RMSE（Root Mean Square Error）
transformed_points = model(locations_1_to_use[ind])
error = transformed_points - locations_2_to_use[ind]
# 计算每个误差向量的欧几里得范数（距离）
...
if model_to_truevalue:
    # 计算 CMR（Correct Match Ratio）
    # cmr = sum(inliers) / len(all_indices) if len(all_indices) > 0 else 0
...
M_all, _ = cv2.estimateAffine2D(locations_1_to_use[ind], locations_2_to_use[ind])
