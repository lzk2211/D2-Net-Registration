...
class D2Net(nn.Module):
    def __init__(self, model_file=None, use_relu=True, use_cuda=True):
        ...
        self.dense_feature_extraction = DenseFeatureExtractionModule(# 特征提取模块
            use_relu=use_relu, use_cuda=use_cuda
        )

        self.detection = HardDetectionModule()# 特征点检测模块

        self.localization = HandcraftedLocalizationModule()# 输出两通道的偏移图，用以对检测到的特征点进行亚像素校正。
